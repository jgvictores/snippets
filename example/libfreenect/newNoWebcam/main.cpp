/*
 * This file is a modified version of a part of the OpenKinect Project
 * (http://www.openkinect.org)
 *
 * Author: Juan G Victores (branched on 2011-03-17)
 *
 * Copyright (c) 2011 Unviersidad Carlos III de Madrid
 * Copyright (c) 2010 individual OpenKinect contributors. See the OpenKinect
 * CONTRIB file for details.
 *
 * This code is licensed to you under the terms of the GNU General Public License,
 * version 2.0 (given the option of GPL2 or the Apache License, version 2.0 from
 ...
 * must accompany it with the GPL2 file. In all cases you must keep the copyright
 * notice intact and include a copy of the CONTRIB file.
 *
 * Binary distributions must follow the binary distribution requirements of the GPL.
 */
...
class MyFreenectDevice : public Freenect::FreenectDevice {
	public:
		...
		bool getDepth(vector<uint8_t> &buffer) {
			...
			if(m_new_depth_frame) {
                ...
                yarp::sig::ImageOf<yarp::sig::PixelRgb>& i_img = p_img.prepare(); 
                i_img.resize(640,480);
                for (int i_x = 0; i_x < i_img.width(); ++i_x) {
                    for (int i_y = 0; i_y < i_img.height(); ++i_y) {
                        p.r = buffer[3*(i_x+(i_y*i_img.width()))];
                        p.g = buffer[1+3*(i_x+(i_y*i_img.width()))];
                        p.b = buffer[2+3*(i_x+(i_y*i_img.width()))];
                        i_img.safePixel(i_x,i_y) = p;
                    }
                }
                p_img.write();
	...
	private:
        BufferedPort<yarp::sig::ImageOf<yarp::sig::PixelRgb> > p_img;
        yarp::sig::PixelRgb p;
...
int main(int argc, char **argv) {
	device = &freenect.createDevice<MyFreenectDevice>(0);
	device->startVideo(); // Here is where libusb tries to open USB device /dev/bus/usb/001/004 and 006
    // Logical, seeing that the output of lsusb on this machine is:
    // Bus 001 Device 006: ID 045e:02ae Microsoft Corp. 
    // Bus 001 Device 005: ID 045e:02ad Microsoft Corp. <-- Corresponds to kinect servo
    // Bus 001 Device 004: ID 045e:02b0 Microsoft Corp. 
