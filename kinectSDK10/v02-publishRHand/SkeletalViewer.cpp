...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
    ...
    LPCWSTR szContents(_T("Click to start YARP..."));
    MessageBox( NULL, szContents, szAppTitle, MB_OK | MB_ICONHAND);
	Network yarp;
	if (!yarp.checkNetwork()) {
		szContents = _T("Failed to find YARP server.");
		MessageBox( NULL, szContents, szAppTitle, MB_OK | MB_ICONHAND);
        return -1;
    }
	g_skeletalViewerApp.port.open("/kinect/skeleton");
	
	szContents = _T("Yay! Started YARP!");
    MessageBox( NULL, szContents, szAppTitle, MB_OK | MB_ICONHAND);
